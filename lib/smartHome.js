...
var sync = require('./sync.json')
    ...
    , anyMesh = new AnyMesh()
    , syncPending = !sync.singleNode; //Set singleNode only to true as long as this node is the only node in the network as it prevents sync at startup which might cause terrible desync issues
...
function startupMesh(cb) {
    anyMesh.networkID = sync.networkName;

...
function startupMesh(cb) {
    ...
    anyMesh.received = function (message) {
        ...
        if (message.target == 'sync') {
            ...
            if (payload.request == 'all') anyMesh.request(message.sender, {"data": sync});
            else if (payload.request == 'device') {
                anyMesh.request(message.sender, {"data": sync[payload.device], "device": payload.device});
            } else if (payload.request == null && payload.data) sync[payload.device] = payload.data;
        }
    };
    ...
    anyMesh.connectedTo = function (info) {
        ...
        if (syncPending) {
            anyMesh.request(info.name, {"request": "all"});
            setTimeout(function () {
                syncSettings("wifi");
            }, 2000);
            syncPending = false;
